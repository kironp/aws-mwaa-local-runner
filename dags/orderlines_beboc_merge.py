...
INT_PIPELINES = [
    'int_orderlines_beboc_order_invoice_lines',
    'int_orderlines_beboc_pnr_product_fare_legs'
]
MERGE_PIPELINES = [
    'merge_orderlines_beboc_order_line_fare_legs'
]
INT_TABLES = [
    'int_beboc_pnr_product_fare_legs',
    'int_beboc_order_invoice_lines'
]

repair_table_part_int = [partial(PythonOperator,
                                 task_id='repair_table.{}.{}'.format(Source.BEBOC, table),
...
full_repair_table_part_merge = [partial(PythonOperator,
                                        task_id='full_repair_table.{}.{}'.format(Source.BEBOC, p),
                                        python_callable=repair_table_callable_filtered_by_source,
                                        op_kwargs={'aws_region': 'eu-west-1',
                                                   'db_name': merge_output_db,
                                                   'source_system': Source.BEBOC.upper(),
...
source_repair_table_part_merge = partial(PythonOperator,
                                         task_id='repair_table.{}.{}'.format(Source.BEBOC, ORDERLINES_TABLE),
                                         python_callable=repair_table_callable_filtered_by_source,
                                         op_kwargs={'aws_region': 'eu-west-1',
                                                    'db_name': merge_output_db,
                                                    'source_system': Source.BEBOC.upper(),
...
beboc_int_meta = PipelineMetaData(
...
beboc_merge_meta = PipelineMetaData(
    project=Project.ORDERLINES,
    stage=Stage.MERGE,
    source=Source.BEBOC,
    spark_job_name=SPARK_JOB_NAME,
...
beboc_int_backfill_meta = PipelineMetaData(
...
beboc_merge_backfill_meta = PipelineMetaData(
    project=Project.BACKFILL_ORDERLINES,
    stage=Stage.MERGE,
    source=Source.BEBOC,
    spark_job_name=SPARK_JOB_NAME,
